...
"""Ephemeral FastAPI server that visualises a compliance-mapping run.
...
opening, and Ctrl-C teardown. This module only implements the endpoints:
    ...
    GET /api/report    → report.html (raw HTML) once Stage 8 completes
    POST /api/verdict  → record a lightweight review verdict
    POST /api/shutdown → ask the launcher to stop uvicorn
...
def build_app(output_dir: Path, *, on_shutdown=None) -> FastAPI:
    ...
    app = FastAPI(
        title="compliance-mapping live server",
    ...
    def api_report() -> HTMLResponse:
        # Serve report.html verbatim — generate_report.py inlines all
        # assets so a raw HTMLResponse is enough; no rewriting needed.
        p = output_dir / "report.html"
        if not p.is_file():
            raise HTTPException(404, detail="report.html not written yet")
        try:
    ...
    def api_verdict(body: dict = Body(...)) -> dict:
        ...
        # alongside the mapping. NOT a review workflow — no assignments,
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", required=True,
                        help="the mapping output dir the agent is writing to")
