...
- **7 risks**, **2 `closed`** (the satisfied checks) / 5 `open`; generic style → rating in an `original-risk-rating` prop, no characterizations; each risk also carries the `rule-set-id` join prop and holds the remediation
...
from the CD props); either way the poam-items reference the group by `rule-set-id` and never duplicate
