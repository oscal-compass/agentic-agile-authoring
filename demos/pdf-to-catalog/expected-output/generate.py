...
CONFIG = {
    "name": "nist-sp-800-171-r3",  # Output file prefix
    "id_prefix": "nist-sp-800-171-r3",  # Control ID prefix
    ...
    "use_pdfplumber": True,
    # TOC and front matter pages (0-indexed). NIST SP 800-171r3:
    #   pp. 1-10 (0-idx 0-9): cover, reviewer note, abstract, TOC, acknowledgments
    #   pp. 11-15 (0-idx 10-14): Section 1 "Introduction" and Section 2 "The Fundamentals"
    #     (narrative background, contains illustrative EXAMPLE control 03.13.11 on p.15)
    #   pp. 16-84 (0-idx 15-83): Section 3 "The Security Requirements" -- BODY
    #   pp. 85+ (0-idx 84+): References, Appendices A-F
    # Skip everything except the body pages 16-84 (0-idx 15-83).
    "toc_pages": list(range(0, 15)) + list(range(84, 120)),
    ...
    "page_number_patterns": [
        ...
        r"^Page\s+\d+",
        # --- NIST SP 800-171r3 running header & date footer (appears on every page) ---
    ...
    "metadata": {
        ...
        "parties": [
            {
                ...
                "type": "organization",
                "remarks": "U.S. federal agency (Department of Commerce) that publishes NIST Special Publications, including SP 800-171r3.",
            }
        ],
        ...
        "props": [
            ...
            {
                "name": "doi",
                "value": "10.6028/NIST.SP.800-171r3",
                "ns": "https://ibm.com/concert/ns/oscal",
            },
        ],
        ...
        "remarks": "OSCAL catalog representation of the security requirements in NIST Special Publication 800-171 Revision 3, 'Protecting Controlled Unclassified Information in Nonfederal Systems and Organizations' (May 2024).",
    },
}
...
PATTERNS = {
    # Chapter level (becomes groups) — NIST SP 800-171r3 uses "3.N. Family Name"
    # as the family (group) marker under Section 3 "The Security Requirements".
    # Example: "3.1. Access Control", "3.17. Supply Chain Risk Management".
    # Group 1 captures the literal "3" (chapter_type — remapped to "family"
    # in parse_structure); group 2 the family number (1..17); group 3 the
    # family title.
    "chapter": r"^(3)\.(\d+)\.\s+([A-Z].*)$",

    # Section level (becomes controls) — NIST SP 800-171r3 uses zero-padded
    # "03.NN.MM Title" for each individual security requirement. Examples:
    # "03.01.01 Account Management", "03.14.06 System Monitoring",
    # "03.14.04 Withdrawn". Title MUST start with an uppercase letter so
    # in-body cross-references like "03.15.03 for authenticators..." do
    # NOT get promoted to phantom controls.
    "section": r"^(03\.\d{2}\.\d{2})\s+([A-Z].+)$",
...
def merge_pages(text_files: list[str], output_path: str) -> str:
    ...
    # Skip TOC / front-matter / back-matter pages. The template's CONFIG
    # documents `toc_pages` but doesn't wire it into merge_pages; we do
    # that here so CONFIG["toc_pages"] (0-indexed page numbers to skip)
    # is honoured. Filenames are shaped `<name>-page-NNN.pdf.txt`, so we
    # extract the page number from the filename and drop pages whose
    # 0-indexed number is in the skip set.
    skip_zero_indexed = set(CONFIG.get("toc_pages", []) or [])
    page_num_re = re.compile(r"-page-(\d+)\.")

    for txt_file in sorted(text_files):
        m = page_num_re.search(os.path.basename(txt_file))
        if m:
            one_indexed = int(m.group(1))
            if (one_indexed - 1) in skip_zero_indexed:
                continue
...
def parse_structure(text: str, page_num: int = 1) -> list[dict]:
    ...
    while i < len(lines):
        ...
        if chapter_match:
            ...
            # NIST SP 800-171r3: chapter regex captures the literal "3" as
            # chapter_type (from headings like "3.1. Access Control").
            # Remap it to "family" so group ids come out as
            # <prefix>-family-<N> instead of <prefix>-3-<N>.
            if chapter_type == "3":
                chapter_type = "family"
        ...
        if section_match:
            ...
            # NIST SP 800-171r3 always places the title (and only the
            # title) on the section line, e.g. "03.04.08 Authorized
            # Software – Allow by Exception". The shared helper's
            # past-tense/participle heuristic falsely classifies titles
            # starting with "Authorized ..." (and similar -ed / -ing
            # first words) as body prose, leaving the title empty and
            # dumping the string into prose. Undo that here: for this
            # document the same-line remainder is ALWAYS the title, so
            # if the helper returned empty title with a non-empty prose
            # seed, promote the seed back to title.
            if not section_title and prose_seed:
                section_title = prose_seed
                prose_seed = ""
            ...
            if not prose_seed:
                ...
                while i < len(lines) and len(title_lines) < max_title_lines:
                    ...
                    if re.match(r"^\([a-z]\)\s+", next_line):  # Lettered item
                        break
                    # NIST-style enumerated items ("a. ", "b. ", "c. ") — these
                    # start every control body in SP 800-171r3 and MUST NOT be
                    # consumed as title continuation.
